...
console.log("Marker dibuat:", umkm.NamaUsaha, lat, lon);
                // Buat marker dengan icon, foto, dan link
                const marker = L.marker([lat, lon], {icon : foodIcon})
            .addTo(map)
            .bindPopup(`
                <b>${umkm.NamaUsaha}</b><br>
                ${umkm.Jenis}<br><br>

                <b>Hari Buka:</b><br>
                ${umkm.Hari_buka || "-"}<br><br>
                ...
                <a href="${umkm["Link Google Maps"]}" target="_blank">
...
function tampilkanDaftar() {
    ...
    daftarUMKM.slice(0, 5).forEach(function (umkm) {
        html += `
        <div onclick="zoomUMKM('${umkm.NamaUsaha}')" style="cursor: pointer; padding: 5px; border-bottom: 1px solid #ccc;">
            ${umkm.NamaUsaha} 
